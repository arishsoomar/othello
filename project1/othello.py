...
# is_valid_move helper function 1
def check_neighbors(board, row, col, opponent):
    ...
    for x,y in list_neighbors:
        # updating current row, col to neighboring row,col vals.
        temp_row = row + x
        temp_col = col + y
        # if the value is inside the grid then return True if one
        # of the values is an opponent.
        if 0 <= temp_row < num_rows and  0 <= temp_col < num_cols:
            if board[temp_row][temp_col] == opponent:
                return True
    return False

# is_valid_move helper function 2
def check_lines(board, row, col, user, opponent):
    ...
    for x, y in list_neighbors:
        # updating temp row, col to neighboring row,col vals.
        temp_row = row + x
        temp_col = col + y

        found_opponent = False
        # while the temp row, col is inside the grid and the value at
        # board[temp_row][temp_col] starts with an opponent, update the
        # temp row, col until it's not an opponent.
...
def get_board_as_string(board):
    '''
    Description:
        This function returns the string representation of the current game 
        board.
    Parameter(s):
        board: A list of lists representing the game board (list).
    Return Value:
...
def update_board(board, pos, color):
    ...
    if color == "white":
        ...
        opponent = 1
        board[row][col] = 2
    # update all the opponents that are being sandwiched to user tokens.
    for x, y in list_neighbors:
        temp_row = row + x
        temp_col = col + y 
